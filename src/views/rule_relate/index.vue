<template>
  ...
  <script>
  import RuleEdit from '@/views/rule_relate/edit'
  export default {
    name: 'rule_control',
    data() {
      return {
    ...
    methods: {
      getData() {
        this.axios.get('/ums-rule-relate/list',response => {
      ...
      handleDelete(id) {
        this.axios.del('/ums-rule-relate/del',response => {
          this.getData()
        },{ id: id })
      },
      ...
      batchdel() {
        if(this.selectedrow.length === 0) {
          ...
          this.axios.del('/ums-rule-relate/batchdel',response => {
            this.getData()
          },{ ids: ids })
        }
      },
      handleBack(id) {
        this.axios.del('/ums-rule-relate/back',response => {
          this.getData()
        }, { id: id }, 'back')
      },
    //   gorole(id) {
